<template>
	<view class="container">
		...
		<view class="agent">
			<view class="head">
				订单总数:<span>{{order_sum.order_count}}</span> 订单总额:<span>{{order_sum.sum_price}}</span>元 
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			this.get_order_data()
			this.$api.CC_request.order_spread_by_date({type:this.current_order_type}).then(res =>{
				this.order_sum = res
			})
		},
		methods: {
			...
			get_order_data(){
				...
				this.loadding = true
				this.$api.CC_request.order_spread_list_by_date(data).then(res => {
				    var data_list = res
					if(res.length > 0){

<template>
  <view class="cash" >
      ...
      <view class="H10"></view>
      <view class='money'>
        <view class='ktx'>可提现金额：{{commissionCount}}元</view>
      </view>
      <view class='jine'>
		<view class="jine_01">¥</view>
		<view class="jine_02"><input type="number" class="uni-input" focus placeholder="请输入金额" v-model="cash_info.money"/></view>
      </view>
	  <view class='khh'>银行名称 &emsp;<input type="text" v-model="cash_info.bankname" /></view>
      <view class='khh'>银行户名 &emsp;<input type="text" v-model="cash_info.name"/></view>
      <view class='khh'>银行卡号 &emsp;<input type="number" v-model="cash_info.cardnum" /></view>
      <view class='txfy'>
        <view class='txfy_01'>
        ...
        <view class='txfy_02'>
          ...
          <view class='txfy_02_r'>1-3个工作日到账</view>
        </view>
      </view>
      <view class='btn' @tap="confim">提现</view>
  </view>
</template>
...
export default {
	  ...
	  methods: {
		  ...
		  confim(){
			this.$api.CC_request.user_submit_cash(this.cash_info).then(res => {
				this.$api.msg("提现成功！")
				this.load_data()
			})
		  }

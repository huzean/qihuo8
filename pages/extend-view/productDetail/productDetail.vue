  ...
  <view class="container">
    ...
    >
      ...
      <view class="tui-header-icon" :style="{ marginTop: top + 'px' }">
        <view
          class="tui-icon tui-icon-arrowleft tui-icon-back"
          :style="{
            color: opcity >= 1 ? '#000' : '#fff',
            background: 'rgba(0, 0, 0,' + iconOpcity + ')',
          }"
          @tap="back"
        ></view>
      </view>
    ...
    <view class="tui-banner-swiper">
      <swiper
        ...
        @change="bannerChange"
      >
        <block v-for="(item, index) in product_info.slider_image" :key="index">
      ...
      <tui-tag type="translucent" shape="circleLeft" size="small"
        >{{ bannerIndex + 1 }} /{{ product_info.slider_image.length }}</tui-tag
      >
    ...
    <view class="tui-pro-detail">
        ...
        <view>
          <view class="tui-pro-pricebox tui-padding">
            <view class="tui-pro-price">
              <view v-if="product_info.vip_price > 0">
                <text class="tui-price">¥{{ product_info.vip_price }}</text>
              </view>
              <view v-else>
                <text class="tui-price">¥{{ product_info.price }}</text>
              </view>
            </view>

            <view
              class="tui-original-price tui-gray"
              style="font-weight: 100; flex-grow: 1"
              v-if="product_info.vip_price > 0"
            >
              价格
              <text class="tui-line-through"
                >￥{{ product_info.ot_price }}</text
              >
            </view>
          </view>
        </view>
      ...
      <view
        class="tui-discount-box tui-radius-all tui-mtop"
          ...
          <view class="tui-tag-coupon-box">
            <template v-for="(item, index) of couponList" v-if="index <= 2">
              <tui-tag
                ...
                type="red"
                shape="circle"
                tui-tag-class="tui-tag-coupon"
                >满{{ item.full }}减{{ item.reduce }}</tui-tag
              >
            </template>
          </view>
          <tui-icon
      ...
      <view
        class="tui-basic-info tui-mtop tui-radius-all"
        v-if="sku_attr.length > 0"
      >
        <view class="tui-list-cell" @tap="showPopup">
          <view class="tui-bold tui-cell-title">已选</view>
          <view class="tui-selected-box">{{ select_product.suk }}</view>
          <tui-icon
            name="more-fill"
            :size="20"
            class="tui-right"
      ...
      <view class="tui-cmt-box tui-mtop tui-radius-all">
        ...
        <view class="tui-cmt-btn">
          <tui-tag
            type="black"
            :plain="true"
            tui-tag-class="tui-tag-cmt"
            @tap="jump_toevaluate(product_info.id)"
            >查看全部评价</tui-tag
          >
        </view>
      </view>
    ...
    <tui-bottom-popup :show="popupShow" @close="hidePopup">
      <view class="tui-popup-box">
        <view class="tui-product-box tui-padding">
          <img :src="select_product.image" class="tui-popup-img" />
          <view class="tui-popup-price">
            <view class="tui-amount tui-bold"
              >￥{{ select_product.price }}</view
            >
            <view class="tui-number"
              ><text v-if="product_info.store_name">{{
                product_info.store_name
              }}</text>
              <text>库存：{{ select_product.stock }}</text>
            </view>
          </view>
        </view>
        <scroll-view scroll-y class="tui-popup-scroll">
          <view class="tui-scrollview-box">
            <!-- <template v-for="(item,index) of sku_attr">
							<view class="tui-bold tui-attr-title">{{item.attr_name}}</view>
							<view class="tui-attr-box">
								<view :class="select_attr_value[index]===i.attr?'tui-attr-item-active':'tui-attr-item'" v-for="(i,j) of item.attr_value"
								 @click="choose_sku_cs(index,i.attr)">
            ...
            <view class="tui-number-box tui-bold tui-attr-title">
              ...
              <tui-numberbox
                :max="select_product.stock"
                :min="1"
                :value="num"
                @change="sku_select_num"
              ></tui-numberbox>
            </view>
          </view>
        </scroll-view>
        ...
        <view
          class="tui-icon tui-icon-close-fill tui-icon-close"
...
export default {
  ...
  onLoad: function (options) {
    ...
    this.$api.CC_request.get_product(this.id).then((res) => {
      this.content = res.storeInfo.description;
      this.product_info = res.storeInfo;
      this.sku_value = res.productValue;
      this.sku_attr = res.productAttr;
      var select_value_key = "";
      for (let [k, v] of Object.entries(res.productAttr)) {
        this.select_attr_value[this.select_attr_value.length] =
          v.attr_values[0];
      }
      if (this.sku_attr.length > 0) {
        this.select_product = this.sku_value[this.select_attr_value.join(",")];
      } else {
        this.select_product = this.product_info;
      }

      this.collected = this.product_info.userCollect;
console.log("1-------1",this.collected)
console.log("2-------2",this.product_info)

    });
    var is_login = uni.getStorageSync("is_login");
    if (is_login == 1) {
      this.$api.CC_request.get_cart_count().then((res) => {
        this.shop_car_num = res.count;
      });
    }
    let obj = {};
    // #ifdef MP-WEIXIN
  ...
  methods: {
    ...
    choose_sku_cs(attr_index, attr_value) {
    ...
    add_cart() {
      this.$api.CC_request.cart_add({
        ...
        console.log("idididi", this.product_info.id);
      });
    },
    //直接购物
    add_shopping() {
      ...
      const arr = this._CreateOrderData(); //组合数据
    },
    //检查规格是否都已选择
    check_sku() {
    ...
    collecting: function () {
      if (this.collected) {
        this.$api.msg("取消收藏");
        this.$api.CC_request.del_fav_product(this.id).then((res) => {});
      } else {
        this.$api.msg("收藏成功");
        this.$api.CC_request.fav_product(this.id).then((res) => {});
      }

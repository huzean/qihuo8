			...
			<view class="tui-product-container">
				<block v-for="(item,index) in productList" :key="index">
					...
					<view class="tui-pro-item" :class="isList?'tui-flex-list':''" hover-class="hover" :hover-start-time="150">
						<image :src="item.image" class="tui-pro-img" :class="isList?'tui-proimg-list':''" @tap="detail(item.id)"/>
						<view class="tui-pro-content">
							<view class="tui-pro-tit">{{item.store_name}}</view>
							<view>
								<view class="tui-pro-price">
									<text class="tui-sale-price" v-if="item.vip_price != undefined">￥{{item.vip_price}}</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			get_pro_cate() {
				this.$api.CC_request.get_products({keyword:this.searchKey,sid:this.sid, cid:this.cid, page:this.pageIndex, limit:this.page_limit}).then(res => {
						...
						product_list.forEach(item=>{     
							...
							cart_list.forEach(cart => {
								if(item.id == cart.product_id){
									...
									this.cart_num[item.id]={cart_id:cart.id,num:cart.cart_num}
								}
							})
						})
						this.productList = this.productList.concat(product_list)
			...
			numberChange(e, index, product) {
				...
				if(current_cart_num == 0){
					this.$api.CC_request.cart_del({ids:this.cart_num[product.id].cart_id}).then(res => {
				...
				if(this.cart_num[product.id].num > 0){
					this.$api.CC_request.cart_update({id:this.cart_num[product.id].cart_id, number:e.value}).then(res => {
						this.update_cart_item_num()
					})
				}else{
				    this.$api.CC_request.cart_add({productId:product.id, cartNum:e.value,'new':0}).then(res => {
					    this.cart_num[product.id]={cart_id:res.cartId,num:current_cart_num}
						this.update_cart_item_num()
				    })
				}				
			},
